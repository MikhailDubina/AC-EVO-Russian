...
2. **Запустите `install_ru.bat`** (или `RUN_install_ru.bat` — окно точно не закроется само) — скрипт найдёт игру в Steam, скопирует файлы переводов и применит патч меню. Если игра в другом месте — введите путь по запросу. Запуск от имени администратора не нужен.
...
**Проверить, что перевод установлен:** запустите `check_ru_installed.bat` (или `RUN_check_ru.bat`). В игре: Настройки → Язык → Русский; в главном меню должны быть «Вождение», «Автомобили», «Академия», «Настройки».
