...
**Вариант без разбора kspkg и без патча меню:** запустите **`install_ru_cn.bat`**. Он копирует русский перевод в файлы **cn.loc**, **cn.cars.loc**, **cn.tooltips.loc**, **cn_extra strings.loc** (подмена китайской локали). В игре выберите язык **«简体中文»** (или «中文») — интерфейс будет на русском. Не нужны Python, parse_kspkg и правка `components.js`. Работает, если игра подхватывает файлы локализации с диска (папка `uiresources\localization` или `content\uiresources\localization`). Если у вас только `content.kspkg` и игра не читает папки с диска — этот вариант может не сработать, тогда используйте `install_ru.bat` с распаковкой.

**Если после install_ru_cn выбран «简体中文», но интерфейс остаётся на китайском:** игра, скорее всего, читает локализацию из пакета `content.kspkg`, а не с диска. Сначала запустите **`install_ru.bat`** — он извлечёт uiresources из kspkg в папку `content\` (или создаст папку с запасным components.js) и скопирует переводы. После этого снова запустите **`install_ru_cn.bat`**, затем в игре выберите «简体中文». Либо используйте только `install_ru.bat` и выберите в меню язык «Русский».
