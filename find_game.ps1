...
if (Test-Path "$def1\\AssettoCorsaEVO.exe") { $result = SafePath $def1 }
elseif (Test-Path "$def2\\AssettoCorsaEVO.exe") { $result = SafePath $def2 }
elseif (Test-Path $vdf) {
    ...
    for ($i = 1; $i -lt $parts.Count; $i++) {
        if ($parts[$i] -match '\\s+"([^"]+)"') {
            ...
            if (Test-Path "$p\\AssettoCorsaEVO.exe") { $result = SafePath $p; break }
        }
    }
}
